...
typedef struct
{
    ...
    u8 Values[]; // List of Huffman values in order of increasing code length
} jpeg_dht;
#pragma pack(pop)
...
function int LoadJPEG(const char* FileName, bitmap* Bitmap);
function int ReadJPEG(void* Data, usz Size, bitmap* Bitmap);
